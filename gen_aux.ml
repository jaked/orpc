...
let gen_aux_mli name (typedefs, excs, funcs, kinds) =
          ...
          let appd =
            let t =
              match name, id with
                | _, "exn" (* hack *)
                | None, _ -> <:ctyp< $lid:id$ >>
                | Some name, _ -> <:ctyp< $uid:name$.$lid:id$ >> in
            G.tapps t (List.map (fun v -> <:ctyp< '$lid:v$ >>) vars) in
            ...
            let ts =
              List.map
                (fun (_, vars, id, t) ->
                  ...
                  TyDcl (_loc, id, vars, G.gen_type t, []))
                ds in
            SgTyp (_loc, tyAnd_of_list ts)
        | Some _ -> <:sig_item< >>$ ;;
      $sgSem_of_list is$
    >> in
  ...
  let gen_func ~has_excs ?name (_, id, args, res) =
    ...
    <:sig_item<
      ...
      $sgSem_of_list
        (List.mapi
            (fun arg i -> <:sig_item< type $lid:G.argi id i$ = $G.gen_type ?name arg$ >>)
...
let rec gen_to ?name t x =
  ...
  match t with
    ...
    | Record (_, fields) ->
        ...
        let rb f e =
          match name with
            | None -> <:rec_binding< $lid:f.f_id$ = $gen_to f.f_typ e$ >>
            | Some name -> <:rec_binding< $uid:name$.$lid:f.f_id$ = $gen_to f.f_typ e$ >> in
        <:expr<
          match Xdr.dest_xv_struct_fast $x$ with
     ...
     | Variant (_, arms) ->
         let mc (id, ts) i =
           let con =
             match name with
               | None -> <:expr< $uid:id$ >>
               | Some name -> <:expr< $uid:name$.$uid:id$ >> in
           match ts with
             | [] -> <:match_case< ($`int:i$, _) -> $con$ >>
             | [t] -> <:match_case< ($`int:i$, x) -> $con$ $gen_to t <:expr< x >>$ >>
             | _ ->
...
let rec gen_of ?name t v =
  ...
  match t with
    ...
    | Record (_, fields) ->
        ...
        let rb f p =
          match name with
            | None -> <:patt< $lid:f.f_id$ = $p$ >>
            (* | Some name -> <:patt< $uid:name$.$lid:id$ = $p$ >> *)(* doesn't work *)
            | Some name -> <:patt< $Ast.IdAcc(_loc, Ast.IdUid (_loc, name), Ast.IdLid (_loc, f.f_id))$ = $p$ >> in
        <:expr<
           ...
           match ts with
             | [] ->
                 let conp =
                   match name with
                     | None -> <:patt< $uid:id$ >>
                     | Some name -> <:patt< $uid:name$.$uid:id$ >> in
                 <:match_case< $conp$ -> Xdr.XV_union_over_enum_fast ($`int:i$, Xdr.XV_void) >>
             | [t] ->
                 let conp =
                   match name with
                     | None -> <:patt< $uid:id$ x >>
                     | Some name -> <:patt< $uid:name$.$uid:id$ x >> in
                 <:match_case< $conp$ -> Xdr.XV_union_over_enum_fast ($`int:i$, $gen_of t <:expr< x >>$) >>
             | _ ->
                 let (pps, pes) = G.vars ts in
                 let conp =
                   match name with
                     | None -> <:patt< $uid:id$ ( $paCom_of_list pps$ ) >>
                     | Some name -> <:patt< $uid:name$.$uid:id$  ( $paCom_of_list pps$ ) >> in
                 <:match_case<
                   $conp$ ->
...
and gen_xdr_def vs bs ds id t =
  <:expr< Xdr.X_rec ($`str:id$, $gen_xdr vs (id::bs) ds t$) >>
...
let gen_aux_ml name (typedefs, excs, funcs, kinds) =
           ...
           (fun (_, vars, id, t) ->
             <:binding<
               ...
               $G.funs_ids
                 ...
                 <:expr< fun x -> $gen_to ?name t <:expr< x >>$ >>$
             >>)
           ds in
           ...
           (fun (_, vars, id, t) ->
             <:binding<
               ...
               $G.funs_ids
                 ...
                 <:expr< fun x -> $gen_of ?name t <:expr< x >>$ >>$
             >>)
           ds in
       StVal (_loc, BTrue, biAnd_of_list es)$ ;;
  ...
  let gen_func ~has_excs ?name (_, id, args, res) =
    ...
    let items aid arg =
      <:str_item<
        type $lid:aid$ = $G.gen_type ?name arg$
        let $lid:G.to_ aid$ x = $gen_to ?name arg <:expr< x >>$
        let $lid:G.of_ aid$ v = $gen_of ?name arg <:expr< v >>$
    ...
    <:str_item<
      ...
      $stSem_of_list
        (List.mapi
            (fun arg i -> <:str_item< type $lid:G.argi id i$ = $G.gen_type ?name arg$ >>)
            (List.map typ_of_argtyp args))$
    >> in
  ...
  <:str_item<
    ...
    $if has_excs
     then
       ...
       <:str_item<
         let to_exn x = $gen_to ?name t <:expr< x >>$
         let of_exn v = $gen_of_exc ?name t <:expr< v >>$
